...
echo "Make directory /var/data/project-plus-all-stars-dolphin/user/Wii if it doesn't exist"
mkdir -p /var/data/project-plus-all-stars-dolphin/Wii

echo "Make directory /var/config/project-plus-all-stars-dolphin if it doesn't exist"
...
UserHDTexturesCreationDate=`stat --format="%W" /var/data/project-plus-all-stars-dolphin/user/Load/Textures/RSBE01`

echo "Check if there are newer HD textures"
if [ $SystemHDTexturesCreationDate -gt ${UserHDTexturesCreationDate:=0} ];
then
    echo "Make directory /var/data/project-plus-all-stars-dolphin/user/Load/Textures if it doesn't exist"
    mkdir -p /var/data/project-plus-all-stars-dolphin/Load/Textures

    echo "Copy newer HD textures to user data directory"
    cp -r /app/share/project-plus-all-stars-dolphin/sys/Load/Textures/RSBE01 /var/data/project-plus-all-stars-dolphin/user/Load/Textures
else
    echo "HD textures are already at latest version"
fi

# Launch Dolphin and point it to the user directory
project-plus-all-stars-dolphin -u /var/data/project-plus-all-stars-dolphin/user "$@"
